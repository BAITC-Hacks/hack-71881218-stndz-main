    ...
    for record in records:
        ...
        if record.name == "get_node":
            lines.append(
                ...
                f"Роль выгрузки: {data['role']}; role_score={data['role_score']}; "
                f"priority_score={data['priority_score']}. "
                f"Входящие: {data['in_kzt']} KZT от {data['in_deg']} плательщиков; "
                f"исходящие: {data['out_kzt']} KZT, {data['out_deg']} получателей."
            )
            ...
            for row in data["collectors"]:
                lines.append(f"{row['gid']}: {row['sum_kzt_from_gids']} KZT от указанных узлов.")
            ...
            lines.extend(f"{row['gid']}: {row['value']} ({data['metric']}); гипотеза — "
...
def edge_line(edge: dict) -> str:
    return f"{edge['source']} → {edge['target']}: {edge['sum_kzt']} KZT, транзакций: {edge['n_tx']}."
