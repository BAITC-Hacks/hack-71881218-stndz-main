...
| `seed_reach2` | разные seed на входящих путях длиной 1–2 | сходимость денег seed |
...
| `fast_share` | доля исходящих сумм, которым предшествовал входящий перевод за 0–2 дня | сквозной транзит |
...
| 5 | `terminal` | колено 1–3, `in_deg > 0`, `retention ≥ 0.8` | 1 158 |
...
`nx.community.louvain_communities(G.to_undirected(), weight="sum_kzt", seed=42)` на NetworkX 3.7 даёт **91 сообщество, 8 из них содержат более одного seed**; все 19 изолированных узлов — отдельные кластеры. Кластеризация работает на **неориентированной** проекции, направление денег учитывается в ролях. `sum_kzt_internal` — сумма рёбер с обоими концами внутри кластера, `top_gids` — до пяти узлов с наибольшим приоритетом.
...
- **Воспроизводимость.** Louvain с `seed=42` и NetworkX 3.7 дают одинаковые выгрузки; тест сравнивает два прогона побайтно. На другой версии NetworkX число кластеров может отличаться.
