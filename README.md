...
На **23 сентября 2026 года** отдельный движок в `analytics/` и `run_pipeline.py` находится на промежуточном этапе; он не заменяет основной `pipeline.py`. Основа и P0-признаки реализованы и отправлены в `main`. Роли, сообщества и приоритеты в текущих выгрузках пока остаются явными заглушками.
...
| PHASE 03 — роли и объяснения | Ожидает решения Ali по D-003 |
| PHASE 04 — сообщества и гипотезы кластеров | Следующая после PHASE 03 |
