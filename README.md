...
На **23 сентября 2026 года** отдельный движок в `analytics/` и `run_pipeline.py` развивается независимо от основного `pipeline.py`. Основа, P0-признаки, роли, объяснения и сообщества реализованы; заглушкой остаётся расчёт приоритетов.

| Фаза | Статус |
|---|---|
| PHASE 00 — аудит и архитектура | Завершена |
| PHASE 01 — загрузка, проверки и граф | Реализация готова; точная версия для воспроизводимости на чистой машине — открытый D-005 |
...
| PHASE 04 — сообщества и гипотезы кластеров | Реализована: 91 кластер, 8 с более чем одним seed; D-005 ещё не закреплён в `requirements.txt` |
| PHASE 05 — временные признаки и приоритет | Требует решения D-008 и завершения предшествующих фаз |
| PHASE 06 — финальные экспорты, тесты и интеграция | Ожидает завершения аналитических фаз и подтверждения расширений JSON-контракта Участником 2 |

Graph Engine выполняет Louvain по **неориентированной проекции** графа с весом `sum_kzt` и `seed=42`; направление потоков учитывается в ролях, не при кластеризации. Числа PHASE 04 получены на локальном NetworkX 3.7: 91 сообщество, 8 с более чем одним seed, все 19 изолированных узлов — отдельные кластеры. Пока D-005 не закреплён в общем `requirements.txt`, одинаковые результаты на чистой машине не гарантированы.
