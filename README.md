...
На **23 сентября 2026 года** отдельный движок в `analytics/` и `run_pipeline.py` развивается независимо от основного `pipeline.py`. Основа, P0-признаки, роли и объяснения реализованы и отправлены в `main`; пока заглушками остаются сообщества и приоритеты.

| Фаза | Статус |
|---|---|
| PHASE 00 — аудит и архитектура | Завершена |
| PHASE 01 — загрузка, проверки и граф | Реализация готова; checkpoint зависит от D-005 — пиновки `networkx==3.7` Участником 3 в общем `requirements.txt` |
...
| PHASE 04 — сообщества и гипотезы кластеров | Следующая; требует точной версии NetworkX по D-005 |
