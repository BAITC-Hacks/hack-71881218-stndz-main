...
Команды выполняются **из корня репозитория**. Нужен **Python 3.12+** (проверенная среда: 3.12.10; проверить: `python --version`): `networkx` зафиксирован на 3.7 ради воспроизводимых кластеров, а эта версия не ставится на Python 3.11 и более ранних версиях; для интерфейса — Node.js 22.12+ и npm. В `data/` должны лежать три исходных `.parquet`. После установки зависимостей аналитика и интерфейс работают локально, LLM и GPU не требуются.
...
```sh
python -m pip install -r requirements.txt && python run_pipeline.py --data-dir ./data --output-dir ./output
```

Для Windows PowerShell 5.1 (без поддержки `&&`):

```powershell
python -m pip install -r requirements.txt; if ($LASTEXITCODE -eq 0) { python run_pipeline.py --data-dir ./data --output-dir ./output }
```
...
Официальный пайплайн создаёт `output/nodes_roles.csv`, `output/clusters.csv`, `output/top_nodes.csv`, `output/graph.json`. Повторный запуск в установленной среде: `python run_pipeline.py`. Команды `npm --prefix frontend run dev` и `run build` копируют свежий JSON в `frontend/public/data/`; CSV находятся в `output/`. Прежний `pipeline.py` сохранён как архивный вариант.
...
Тесты проверяют аналитические функции, API и выгрузки. Интеграционные проверки запускают `run_pipeline.py` во временные каталоги, проверяют лимит 300 секунд, побайтный детерминизм CSV, полноту узлов и кластеров, скоры, evidence, топ и ограничения данных. Рабочие выгрузки интерфейса при этом не перезаписываются.

**Проверка объединённого проекта:** 130 тестов прошли. Прежние ошибки evidence и seed/transit относятся к архивному `pipeline.py`; официальный движок проходит эти проверки. Подробности — в [delivery-статусе](docs/DELIVERY_STATUS.md).

## Статус Graph Intelligence Engine — Участник 1

На **23 сентября 2026 года** официальный движок — `analytics/` и `run_pipeline.py`. Основа, P0-признаки, роли, объяснения, сообщества, временные признаки и приоритет реализованы. API нормализует его JSON, интерфейс читает его через `graphAdapter.js`.

| Фаза | Статус |
|---|---|
| PHASE 00 — аудит и архитектура | Завершена |
| PHASE 01 — загрузка, проверки и граф | Реализована; `networkx==3.7` закреплён по D-005, требуется Python 3.12+ |
| PHASE 02 — структурные и денежные признаки | Реализована, smoke-проверки пройдены |
| PHASE 03 — роли и объяснения | Реализована; 2 248 узлов классифицированы, D-003 принят |
| PHASE 04 — сообщества и гипотезы кластеров | Реализована на закреплённом NetworkX 3.7: 91 кластер, 8 с более чем одним seed |
| PHASE 05 — временные признаки и приоритет | Реализована; D-008 принят (вариант б), 30 top-node рангов сформированы |
| PHASE 06 — финальные экспорты, тесты и интеграция | Реализована и проверена; ожидается подтверждение полей `[ADD]` / `[CLARIFY]` Участником 2 |

Graph Engine выполняет Louvain по **неориентированной проекции** графа с весом `sum_kzt` и `seed=42`; направление потоков учитывается в ролях, не при кластеризации. D-005 закрыт: в общем `requirements.txt` закреплён NetworkX 3.7. На нём получено 91 сообщество, 8 с более чем одним seed; все 19 изолированных узлов — отдельные кластеры.

D-008 принят по варианту б: веса `flow=0.25`, `structure=0.20`, `volume=0.20`, `seed_conv=0.10`, `external=0.15`, `temporal=0.10`; seed получают множитель `0.8`, а `boundary` / `peripheral` / isolated ограничиваются `0.3`. `fast_share` считает исходящие суммы с наблюдаемым входящим переводом в предыдущие 0–2 дня; `sync_in_max` — максимум разных плательщиков за календарный день. Топ-30 на текущих данных не содержит `boundary`.

### Правила ролей Graph Engine

Эта таблица относится к официальному движку `analytics/`. Алгоритм прежнего `pipeline.py` сохранён ниже в архивном разделе.

| Порядок | Роль | Условие | Узлов |
|---:|---|---|---:|
| 1 | `coordinator` | `in_deg ≥ 5` и `out_deg ≥ 10` | 15 |
| 2 | `distributor` | `out_deg ≥ 10` | 49 |
| 3 | `consolidator` | `in_deg ≥ 5`, или `in_deg ≥ 3` и `seed_reach2 ≥ 3`, или `in_kzt ≥ 613 779 KZT` и `in_deg ≥ 3` | 69 |
| 4 | `transit` | не seed; `in_deg > 0`, `out_deg > 0`, `0.8 ≤ pass_through ≤ 1.2` | 67 |
| 5 | `terminal` | `depth < 4`, `in_deg > 0`, `retention ≥ 0.8` | 1 158 |
| 6 | `boundary` | `depth = 4`, `out_deg = 0` | 444 |
| 7 | `peripheral` | ни одно предыдущее правило не сработало | 446 |

Удержание рассчитывается как `1 − pass_through`; правило `terminal` не применяется к границе обхода. `role_score` — эвристическая уверенность в структурном соответствии правилу, не вероятность нарушения. У seed evidence отдельно предупреждает о неполноте входящих данных.

Подробности: [план движка](docs/GRAPH_ENGINE_PLAN.md), [журнал прогресса](docs/GRAPH_ENGINE_PROGRESS.md), [решения и блокеры](docs/GRAPH_DECISIONS.md).
...
<details>
<summary>Архив: признаки, роли и приоритеты прежнего pipeline.py</summary>

## Признаки и критерии ролей прежнего пайплайна

Ниже описан **прежний `pipeline.py`**. Этот раздел не относится к официальному `run_pipeline.py`; актуальные правила приведены в разделе Graph Intelligence Engine выше.
...
| Файл | Содержание |
|---|---|
| `output/nodes_roles.csv` | 2 248 строк; обязательные `gid, role, role_score, cluster_id, priority_score, evidence`, дополнительные колонки с признаками |
...
| `output/graph.json` | Контракт движка: `meta, nodes, edges, clusters` |
| `frontend/public/data/graph.json` | Копия JSON, обновляемая скриптом `sync-data` при запуске/сборке UI |

Официальный JSON использует строковые `nodes[].gid`, `edges[].src/dst`, вложенные `metrics` и `flags`, а также `rank` узла. Backend и frontend нормализуют этот формат для своих потребителей. Полный контракт — [docs/GRAPH_DATA_CONTRACT.md](docs/GRAPH_DATA_CONTRACT.md). Отдельные транзакции и дневной временной ряд в текущий JSON движка не включены; суммы связей агрегированы за период.
...
4. Переходите по контрагентам из агрегатов связей. Для официальной выгрузки показывайте наблюдаемые суммы и число переводов; подробная история зависит от наличия транзакционных данных в используемом JSON.
...
- **Временные признаки эвристические.** Официальный движок рассчитывает `ext_inflow`, `seed_reach2`, `fast_share` и `sync_in_max`. Близость дат входа и выхода не доказывает движение одних и тех же денег.
...
`GraphStore` ищет `output/graph.json` (движок `run_pipeline.py`), затем `out/graph.json` и копию `frontend/public/data/graph.json`; файл можно задать явно через `MONEYGRAPH_GRAPH_JSON`. Читаются оба формата выгрузки. Данные загружаются при старте; после пересчёта нужен перезапуск сервера. Если файла нет, сначала выполните пайплайн.
...
Изымает `n` (1–100) самых значимых узлов и сравнивает связность сети до и после: число компонент, размер крупнейшей, seed в ней. Значимость — `priority_score`, пока он не рассчитан — `betweenness`; поле `ranked_by` показывает, что использовано. Связность считается без учёта направления переводов. На текущей выгрузке изъятие 10 узлов по betweenness разбивает сеть с 35 до 228 компонент, крупнейшая сокращается с 1 877 до 1 567 узлов.
...
Подробный контракт, лимиты и примеры вопросов: [docs/COPILOT_API.md](docs/COPILOT_API.md). В React подключена панель Copilot и прокси `/api` на FastAPI; сам граф продолжает загружаться из статического JSON.
