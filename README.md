...
На **23 сентября 2026 года** отдельный движок в `analytics/` и `run_pipeline.py` развивается независимо от основного `pipeline.py`. Основа, P0-признаки, роли, объяснения, сообщества, временные признаки и приоритет реализованы.
...
Эта таблица относится к отдельному движку `analytics/`, а не к фактическим правилам основного `pipeline.py`, описанным ниже.
...
Без ключа сразу работает разбор типовых вопросов по правилам. Пример в PowerShell:
...
Для LLM с OpenAI-совместимым Chat Completions и поддержкой tool calling задайте переменные **перед запуском сервера**:

| Переменная | Значение |
|---|---|
| `LLM_BASE_URL` | Базовый URL API с префиксом, например `https://api.openai.com/v1`; приложение добавляет `/chat/completions` |
| `LLM_API_KEY` | Ключ провайдера; без ключа сетевые запросы к LLM не выполняются |
